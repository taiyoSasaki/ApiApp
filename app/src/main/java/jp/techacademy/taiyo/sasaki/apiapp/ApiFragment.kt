...
class ApiFragment: Fragment() {
    ...
    //Apiでデータを読み込み中ですプラグ。追加ページの読み込みの時にこれがないと、連続して読み込んでしまうので、それの制御のため
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        recyclerView.apply {
            ...
            addOnScrollListener(object: RecyclerView.OnScrollListener() { //Scrollを検知するListenerを実装する。これによって、RecyclerViewの下端に近づいた時に次のページを読み込んで、下に付け足す
                override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) { //dxはx軸方向の変化量(横) dyはy軸方向の変化量(縦)ここではRecyclerViewは縦方向なのでdyだけ考慮する
                    ...
                    //totalCountとlastVisibleItemから全体のアイテム数のうちどこまでが見えているかがわかる(例:totalCountが20、lastVisibleItemが15の時は現在のスクロール位置から下に5件見えていないアイテムがある)
                    //一番下にスクロールしたときに次の20件を表示する等の実装が可能となる
                    //ユーザビリティを考えると、一番下にスクロールしてから追加した場合、一度スクロールが止まるので、ユーザーは気づきにくい
                    //ここでは、一番下から5番目を表示したときに追加読み込みするように実装する
                    if(!isLoading && lastVisibleItem >= totalCount - 6) { //読み込み中出ない、かつ、現在のスクロール位置から下に5件見えていないアイテムがある
                        updateData(true)
                    }
                }
    ...
    fun updateView() { //お気に入りが削除されたときにの処理(Activityからコールされる)
